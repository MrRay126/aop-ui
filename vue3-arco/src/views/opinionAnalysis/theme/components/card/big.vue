            ...
            <template v-if="wordTable">
              ...
              <div class="two-tables-container">
                ...
                <el-table
                  ...
                  :highlight-current-row="true"
                >
                  <el-table-column prop="word" label="口碑词">
                  </el-table-column>
...
export default {
  ...
  watch: {
    cardData: {
      immediate: true,
      handler(val) {
        if (val && val.wordTypesTableData) {
          this.isViewInfos = true;

          this.wordTypesTableData = val.wordTypesTableData;
          if (this.wordTypesTableData.length > 0) {
            //设置点击
            this.$nextTick(() => {
              this.handleKeyWordRowClick(this.wordTypesTableData[0]);
            });
          }
        }
      },
