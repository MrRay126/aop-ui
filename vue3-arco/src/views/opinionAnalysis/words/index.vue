<template>
  <div class="theme" ref="words" v-loading="isLoadingReportConfigs">
              ...
              <div class="right-section">
                <div class="top-tools">
                  ...
                  >
                    ...
                    <el-radio-button :disabled="tableModeDisabled" label="table"
                      >列表模式</el-radio-button
                    >
                  </el-radio-group>
                </div>
                <div class="bottom-container" ref="container">
                  ...
                  <template v-else-if="hasData">
                    <template>
                      <!-- 关键词chart -->
                      <div
                        class="table-container-first"
                        v-show="mode === 'word'"
                      >
                        <div class="title">
                          <!-- 占位元素 -->
                          <span class="view-word-title">点击词 :</span>
                          <span class="view-word-name">{{
                    ...
                    <template>
                      <div class="table-mode-wraper" v-show="mode === 'table'">
                        <table-mode
                          ref="tableMode"
          ...
          <div class="detail-table-container">
            <detail-table
              ...
              :configs="{
                ...
                // opinion: this.dispositionRow.word,
...
import { DISPOSITION, getDimension } from "./data.js";
...
export default {
  ...
  data() {
    return {
      ...
      pageTitle: "词云分析",
      comment:
        "分析某段时间内，去除停用词后的高频词情感倾向。备注：词的大小与其统计数量相关，颜色越红说明该词负面情绪占比越高。",
  ...
  methods: {
    ...
    handleDownloadDetail() {
      ...
      const payload = {
        urlParams: { download_type: "words_detail" },
    ...
    handleWordCloudItemClick(word) {
      ...
      this.getDispositionTableData(word.name);
    },
    ...
    getDispositionTableData(word) {
      ...
      this.$store
        ...
        .then((data) => {
          //评价词
          this.wordTable = data || {};
          // this.tableFilterOptions = data.filterOptions || {};
        })
        .catch(() => {
          //noop
        })
    ...
    handleCalculate() {
      ...
      this.mode = "word";
      this.calculateWord(); // word mode

      // 获取table模式的数据,获取完成之前disabled
      this.getFirstTableData(); //table mode
    },
    ...
    async calculateWord() {
      ...
      try {
        ...
        this.alreadyCalculated = false;
        resData = await this.$store.dispatch("word/calculateWords", payload);
      } catch (err) {
      ...
      if (resData.status === 3) {
        ...
        setTimeout(() => {
          ...
          this.wordCloudChartData = resData.words;
      ...
      function initTableData() {
        ...
        if (this.wordCloudChartData.length > 0) {
          const firstWord = this.wordCloudChartData[0];
