<template>
  <div class="hot-words">
    ...
    <word-group-dialog
      ...
      @createHotword="handleCreateHotword"
    ></word-group-dialog>
    ...
    <!-- <hotword-list-dialog
      ref="hotWordList"
      :visible="isHotwordListDialogVisile"
      @close="handleCloseWordList"
...
<script>
import wordGroupDialog from "./components/wordGroupDialog.vue";
import hotwordListDialog from "./components/hotwordListDialog.vue";
...
export default {
  ...
  data() {
    return {
      ...
      isHotwordListDialogVisile: false,
  ...
  methods: {
    ...
    handleWordSubmit() {
      if (this.isHotwordListDialogVisile) {
        this.$refs["hotWordList"].getTableData();
      }
    },
    ...
    handleCloseWordList() {
      this.isHotwordListDialogVisile = false;
    },

    handleOpenWordList() {
      this.isHotwordListDialogVisile = true;
    },
    ...
    handleDeleteRows() {
      ...
      this.$confirm("确定要删除选中的词组吗?", "提示")
        .then(async () => {
          ...
          await this.deleteWordGroup(payload);
